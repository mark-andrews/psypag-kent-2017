beta.hpd.interval <- function(alpha, beta, hpd.mass=0.95){
  ...
  err.fn <- function(p.star, this.hpd.mass=hpd.mass){
    return((this.hpd.mass-interval.mass(p.star))^2)
  }
...
beta.hpd.segment <- function(alpha, beta, hpd.mass=0.95){
  
  HPD <- beta.hpd.interval(alpha, beta, hpd.mass)
  
  hpd.interval <- HPD$hpd.interval
  p.star <- HPD$p.star
  
  c(x=hpd.interval[1], y=p.star, xend=hpd.interval[2], yend=p.star)
